...
# Hijos bash -c con pipefail (M15): sh es dash en varias distros y
# `set -o pipefail` ahi aborta el -c; con bash el match parcial que
# luego falla ya no da PASS en falso.
...
t "doctor reporta nivel" -- bash -c 'set -o pipefail 2>/dev/null; arxy doctor | grep -q "nivel [12]"'
LEVEL="$(arxy doctor 2>/dev/null | grep -o 'nivel [12]' | head -1)"
echo "INFO: detectado $LEVEL"
t "setup file://" -- arxy setup
t "setup deja rootfs valido" -- bash -c "set -o pipefail 2>/dev/null; test -x '$R/usr/bin/pacman' && test -f '$R/etc/arch-release'"
t "run true" -- arxy run /usr/bin/true
t "bash --version" -- bash -c 'set -o pipefail 2>/dev/null; arxy run /usr/bin/bash --version | grep -q "GNU bash"'
t "pacman --version (carga libalpm+glib)" -- bash -c 'set -o pipefail 2>/dev/null; arxy run /usr/bin/pacman --version | grep -q Pacman'
...
t "which distingue origen" -- bash -c 'set -o pipefail 2>/dev/null; arxy which bash | grep -q subsistema'
t "info con contenido" -- bash -c 'set -o pipefail 2>/dev/null; arxy info bash | grep -q "^Name *: bash"'
t "list con contenido" -- bash -c 'set -o pipefail 2>/dev/null; arxy list | grep -q "^pacman "'
t "search con contenido" -- bash -c 'set -o pipefail 2>/dev/null; arxy search nano | grep -q "extra/nano"'
...
t "L2: run true" -- bash -c 'set -o pipefail 2>/dev/null; ARXY_LEVEL=2 arxy run /usr/bin/true'
t "L2: info con contenido" -- bash -c 'set -o pipefail 2>/dev/null; ARXY_LEVEL=2 arxy info bash | grep -q "^Name *: bash"'
t "L2: AUR bloqueado con mensaje" -- bash -c 'set -o pipefail 2>/dev/null; ARXY_LEVEL=2 arxy install --aur nano 2>&1 | grep -q "necesita nivel 1"'
...
t "export sintetico" -- bash -c "set -o pipefail 2>/dev/null; arxy export --all >/dev/null && test -f '$APPS/arxy-arxy-test.desktop'"
t "export contenido" -- bash -c "set -o pipefail 2>/dev/null; grep -q '^Exec=arxy run /usr/bin/true' '$APPS/arxy-arxy-test.desktop' && grep -q '^X-Arxy-Pkg=' '$APPS/arxy-arxy-test.desktop'"
t "unexport" -- bash -c "set -o pipefail 2>/dev/null; arxy unexport arxy-test && test ! -f '$APPS/arxy-arxy-test.desktop'"
...
t "L2: export --all" -- bash -c "set -o pipefail 2>/dev/null; ARXY_LEVEL=2 arxy export --all >/dev/null && test -f '$APPS/arxy-arxy-test.desktop'"
t "L2: export contenido" -- bash -c "set -o pipefail 2>/dev/null; grep -q '^Exec=arxy run /usr/bin/true' '$APPS/arxy-arxy-test.desktop' && grep -q '^X-Arxy-Pkg=' '$APPS/arxy-arxy-test.desktop'"
t "L2: unexport" -- bash -c "set -o pipefail 2>/dev/null; arxy unexport arxy-test && test ! -f '$APPS/arxy-arxy-test.desktop'"
t "install (escritura)" -- arxy install tree
t "run instalado" -- bash -c 'set -o pipefail 2>/dev/null; arxy run tree --version | grep -qE "tree v[0-9]"'
t "remove" -- arxy remove tree
t "fc-list con contenido" -- bash -c 'set -o pipefail 2>/dev/null; arxy run /usr/bin/fc-list | grep -q "\.ttf"'
t "clean dry-run no toca" -- bash -c 'set -o pipefail 2>/dev/null; arxy clean | grep -q "dry-run"'
t "quickstart guia" -- bash -c 'set -o pipefail 2>/dev/null; arxy quickstart | grep -q "siguiente paso"'
t "doctor --fix informa" -- bash -c 'set -o pipefail 2>/dev/null; arxy doctor --fix 2>&1 | grep -q "\[OK\]\|\[FALTA\]"'
t "doctor --json format" -- bash -c 'set -o pipefail 2>/dev/null; arxy doctor --json 2>/dev/null | grep -q "\"format\": 1"'
t "gc --json format" -- bash -c 'set -o pipefail 2>/dev/null; arxy gc --json 2>/dev/null | grep -q "\"format\": 1" && arxy gc --json 2>/dev/null | grep -q "\"total_bytes\":" && arxy gc --json 2>/dev/null | grep -q "\"applied\": false"'
...
t "rollback sin .old falla limpio" -- bash -c 'set -o pipefail 2>/dev/null; rm -rf "$ARXY_ROOT.old"; ! arxy rollback 2>/dev/null'
if [[ "$LEVEL" == "nivel 1" ]]; then
    # Autodeteccion de nivel 2 con bwrap roto (solo lectura).
    mkdir -p /tmp/nobwrap
    printf '#!/bin/sh\nexit 1\n' > /tmp/nobwrap/bwrap
    chmod +x /tmp/nobwrap/bwrap
    t "L2 autodetectado sin bwrap" -- bash -c 'set -o pipefail 2>/dev/null; PATH="/tmp/nobwrap:$PATH" arxy doctor | grep -q "nivel 2"'
    t "L2 autodetectado corre" -- bash -c 'set -o pipefail 2>/dev/null; PATH="/tmp/nobwrap:$PATH" arxy run /usr/bin/true'
    rm -rf /tmp/nobwrap
else
...
if [[ -n "${MATRIX_WRITE2:-}" ]]; then
    ...
    t "L2: install/remove (chroot)" -- bash -c 'set -o pipefail 2>/dev/null; ARXY_LEVEL=2 arxy install tree && ARXY_LEVEL=2 arxy run tree --version | grep -qE "tree v[0-9]" && ARXY_LEVEL=2 arxy remove tree'
    ...
    t "L2: export --all tras install (chroot)" -- bash -c "set -o pipefail 2>/dev/null; ARXY_LEVEL=2 arxy export --all >/dev/null && grep -q '^X-Arxy-Pkg=' '$APPS/arxy-arxy-test.desktop'"
    t "L2: unexport tras install (chroot)" -- bash -c "set -o pipefail 2>/dev/null; arxy unexport arxy-test && test ! -f '$APPS/arxy-arxy-test.desktop'"
    ...
    t "L2: instala paquete con .desktop (chroot)" -- bash -c 'set -o pipefail 2>/dev/null; ARXY_LEVEL=2 arxy install xterm && test -f "$ARXY_ROOT/usr/share/applications/xterm.desktop"'
    t "L2: export del .desktop real" -- bash -c "set -o pipefail 2>/dev/null; ARXY_LEVEL=2 arxy export xterm >/dev/null && grep -q '^Exec=arxy run /usr/bin/xterm' '$APPS/arxy-xterm.desktop' && grep -q '^X-Arxy-Pkg=xterm' '$APPS/arxy-xterm.desktop' && test -f '$APPS/arxy-uxterm.desktop'"
    t "L2: remove limpia sus lanzadores" -- bash -c 'set -o pipefail 2>/dev/null; ARXY_LEVEL=2 arxy remove xterm >/dev/null && test ! -f "$APPS/arxy-xterm.desktop" && test ! -f "$APPS/arxy-uxterm.desktop"'
else
...
t "rollback restaura setup anterior" -- bash -c 'set -o pipefail 2>/dev/null; 
...
t "clean --apply borra rollback" -- bash -c 'set -o pipefail 2>/dev/null; test -d "$ARXY_ROOT.old" && arxy clean --apply | grep -q "limpieza hecha" && test ! -d "$ARXY_ROOT.old"'
...
t "limpia residuo en REAL_APPS" -- bash -c "set -o pipefail 2>/dev/null; ls '$APPS'/arxy-*.desktop 2>/dev/null | sort >'$SNAP_F.cur' || true; extra=\"\$(comm -13 '$SNAP_F' '$SNAP_F.cur')\"; printf '%s\n' \"\$extra\" | while IFS= read -r f; do test -z \"\$f\" || rm -f \"\$f\"; done; ls '$APPS'/arxy-*.desktop 2>/dev/null | sort >'$SNAP_F.cur' || true; rest=\"\$(comm -13 '$SNAP_F' '$SNAP_F.cur')\"; printf '%s\n' \"\$rest\"; test -z \"\$rest\""
